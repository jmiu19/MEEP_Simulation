...
def simulation(params):
    ...
    nwvg_up = params['Nwvg_ups']           # number of waveguide periods in the upper cavity
    nwvg_lo = params['Nwvg_los']           # number of waveguide periods in the lower cavity
    ...
    center_right_up = - x_taper[-1] - (nwvg_up-0.5)*a_0
    center_left_up  = + x_taper[-1] + (nwvg_up-0.5)*a_0
    sx_up  = 4*center_left_up
    sx_low = 2*sum(a_taper) + 2*(nwvg_lo-1)*a_0 + a_0   # length of the lower cavity
    sx = max(sx_up, sx_low)
    ...
    if (NULL):
        ...
        for center in [center_left_up, center_right_up]:    
            for mm in range(nwvg_up):
                geometry.append(mp.Cylinder(material=mp.air, radius=r_0*a_0, height=mp.inf,
                                            center=mp.Vector3(center+sum(a_taper)-a_taper[-1]/2+mm*a_0, (w*a_0+sep)/2, 0)))
                geometry.append(mp.Cylinder(material=mp.air, radius=r_0*a_0, height=mp.inf,
                                            center=mp.Vector3(center-sum(a_taper)+a_taper[-1]/2-mm*a_0, (w*a_0+sep)/2, 0)))
            ...
            for mm in range(Ndef):
                geometry.append(mp.Cylinder(material=mp.air, radius=r_taper[mm], height=mp.inf,
                                            center=mp.Vector3(center+x_taper[mm], (w*a_0+sep)/2, 0)))
                geometry.append(mp.Cylinder(material=mp.air, radius=r_taper[mm], height=mp.inf,
                                            center=mp.Vector3(center-x_taper[mm], (w*a_0+sep)/2, 0)))

        ## LOWER band
        ## add holes (waveguide) to the band
        for mm in range(nwvg_lo):
            geometry.append(mp.Cylinder(material=mp.air, radius=r_0*a_0, height=mp.inf,
                                        center=mp.Vector3(+sum(a_taper)-a_taper[-1]/2+mm*a_0, (-1)*(w*a_0+sep)/2, 0)))
            geometry.append(mp.Cylinder(material=mp.air, radius=r_0*a_0, height=mp.inf,
                                        center=mp.Vector3(-sum(a_taper)+a_taper[-1]/2-mm*a_0, (-1)*(w*a_0+sep)/2, 0)))
        ## add holes (taper) to the band
        for mm in range(Ndef):
            geometry.append(mp.Cylinder(material=mp.air, radius=r_taper[mm], height=mp.inf,
                                        center=mp.Vector3(+x_taper[mm], (-1)*(w*a_0+sep)/2, 0)))
            geometry.append(mp.Cylinder(material=mp.air, radius=r_taper[mm], height=mp.inf,
                                        center=mp.Vector3(-x_taper[mm], (-1)*(w*a_0+sep)/2, 0)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-params', type=str, default='[60,0.05,0.6,0.85,True,300,3,10]',
