...
def main(args):
    ...
    sep = args.sep                        # separation between edges of the two bands (0.1 um)
    ...
    sx = 2*sum(x_taper) + 2*(Nwvg-1)*a_0 + a_0       # length of the crystal cavity
    ...
    for i in [0,1]:
        for mm in range(Nwvg):
            ...
            geometry.append(mp.Cylinder(material=mp.air, radius=r_0*a_0, height=mp.inf,
                                        center=mp.Vector3(-sum(x_taper)+a_taper[-1]/2-mm*a_0, ((-1)**i)*(w*a_0+sep)/2, 0)))
    ...
    sim.run(mp.in_volume(mp.Volume(center=mp.Vector3(), size=mp.Vector3(sx,sy,0)), mp.at_end(mp.output_epsilon, mp.output_efield_y)),
            mp.in_volume(mp.Volume(center=mp.Vector3(0, +(w*a_0+sep)/2, 0), size=mp.Vector3(sx,sy,0)), mp.at_end(mp.output_epsilon, mp.output_efield_y)),
            mp.in_volume(mp.Volume(center=mp.Vector3(0, -(w*a_0+sep)/2, 0), size=mp.Vector3(sx,sy,0)), mp.at_end(mp.output_epsilon, mp.output_efield_y)),
...
if __name__ == '__main__':
    ...
    parser.add_argument('-Nwvg', type=int, default=10, help='number of waveguide periods (default: 10)')
    ...
    parser.add_argument('-sep', type=float, default=0.1, help='edge to edge separation of bands (default: 0.1)')
