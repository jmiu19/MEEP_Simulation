...
def simulation(params):
    ...
    nwvg_0 = params['Nwvg_0']              # number of waveguide periods in the middle cavity
    Nwvgs = [nwvg_up, nwvg_0, nwvg_up]
    ...
    sx = 2*sum(a_taper) + 2*(max(nwvg_up, nwvg_lo)-1)*a_0 + a_0   # length of the crystal cavity
    ...
    geometry = [mp.Block(material=SiN, center=mp.Vector3(0, +(w*a_0+sep)/2, 0), size=mp.Vector3(mp.inf,w*a_0,h)),
                mp.Block(material=SiN, center=mp.Vector3(0, 0, 0), size=mp.Vector3(mp.inf,w*a_0,h)),
        ...
        for i in [0,-1,1]:
            ...
            for mm in range(Nwvg):
                geometry.append(mp.Cylinder(material=mp.air, radius=r_0*a_0, height=mp.inf,
                                            center=mp.Vector3(+sum(a_taper)-a_taper[-1]/2+mm*a_0, (i)*(w*a_0+sep)/2, 0)))
                geometry.append(mp.Cylinder(material=mp.air, radius=r_0*a_0, height=mp.inf,
                                            center=mp.Vector3(-sum(a_taper)+a_taper[-1]/2-mm*a_0, (i)*(w*a_0+sep)/2, 0)))
            ...
            for mm in range(Ndef):
                geometry.append(mp.Cylinder(material=mp.air, radius=r_taper[mm], height=mp.inf,
                                            center=mp.Vector3(+x_taper[mm], (i)*(w*a_0+sep)/2, 0)))
                geometry.append(mp.Cylinder(material=mp.air, radius=r_taper[mm], height=mp.inf,
                                            center=mp.Vector3(-x_taper[mm], (i)*(w*a_0+sep)/2, 0)))
