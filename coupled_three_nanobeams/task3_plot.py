...
df.columns = ['lam_up', 'lam_low', 'nu', 'Q-factor']
...
plt.scatter(df['lam_low'], df['Q-factor'], s=55, label="Q-factor", marker="+", color="darkorange")
plt.plot(df['lam_low'], df['Q-factor'], alpha=0.3, color="blue")
plt.legend()
plt.grid()
plt.xlabel(r"source wavelength (lower bound) [$\mu$m]", fontsize=14)
...
plt.figure(figsize=(5,3))
plt.scatter(df['lam_low'], df['lambda'], s=55, label="lambda", marker="+", color="darkorange")
plt.plot(df['lam_low'], df['lambda'], alpha=0.3, color="red")
plt.legend()
plt.grid()
plt.xlabel(r"source wavelength (lower bound) [$\mu$m]", fontsize=14)
