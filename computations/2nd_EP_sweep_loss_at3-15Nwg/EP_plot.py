...
fit_decay = np.linspace(df['decay'].values.tolist()[0]*1.05,
                        df['decay'].values.tolist()[-1]*0.9, 100)
fig, ax = plt.subplots(figsize = (6,4), dpi = 150)
...
fig, ax = plt.subplots(3, 1, sharex=True, figsize=(6,9), dpi=150)
for j in range(2):
    for i in range(len(realCompEigVals[j])):
        if (j==1 & i==1):
            ax[j].plot(Gammas, realCompEigVals[j][i],
                   color='orange', alpha=0.3, label='theory (with loss contrast)')
            ax[j].plot(Gammas, HrealCompEigVals[j][i],
                   color='blue', alpha=0.3, label='theory (w/o loss constrast)')
        else:
            ax[j].plot(Gammas, realCompEigVals[j][i],
    ...
    for decay in df['lossDiff']:
        ax[j].axvline(x=decay, color='gray', alpha=0.1)
    ax[j].axvspan (-2*targetC, Gammas[1], color='red', alpha=0.1, lw=0)
    ax[j].axvspan (Gammas[-1], -2*targetC, color='green', alpha=0.1, lw=0)

ax[0].scatter(sim_df['lossDiff'], sim_df['freq'], color='orange')
ax[1].scatter(sim_df['lossDiff'], -sim_df['decay'], 
              color='orange',label='simulation')
ax[2].scatter(sim_df['lossDiff'], 1/sim_df['freq'], color='black')
for i in range(len(realCompEigVals[0])):
    ax[2].plot(Gammas, 1/np.array(realCompEigVals[0][i]), 
               color='black', alpha=0.3)
for decay in df['lossDiff']:
    ax[2].axvline(x=decay, color='gray', alpha=0.1)
